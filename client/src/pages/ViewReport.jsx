...
const ViewReports = () => {
    ...
    const fetchReports = async () => {
        try {
            const reportsResponse = await axios.get('https://excel-analytics-new.onrender.com/api/analytics/reports');
            const reports = reportsResponse.data;

            const reportsWithCharts = await Promise.all(
                reports.map(async (report) => {
                    try {
                        const chartResp = await axios.get(`https://excel-analytics-new.onrender.com/api/chart-analytics/${report._id}`);
                        return {
                            ...report,
    ...
    const generateInsight = async (chartId, chartData, chartType) => {
        ...
        try {
            ...
            const response = await axios.post('https://excel-analytics-new.onrender.com/api/insights/generate', payload);
